...
import VideoInterview from "@/components/VideoInterview"; // Import Live Video Component

export default function Dashboard() {
  const router = useRouter();
  const [activeTab, setActiveTab] = useState<"jobs" | "resumes" | "applications" | "video">("jobs");
  const [jobs, setJobs] = useState([]);
  const [refreshResumes, setRefreshResumes] = useState(false);
  const [isAuthenticated, setIsAuthenticated] = useState(false);
  const roomId = "hiring-platform-" + Math.random().toString(36).substr(2, 9); // Unique room ID
    ...
    <div className="p-6">
      ...
      <div className="flex justify-center gap-4 mb-6">
        ...
        <button
          onClick={() => setActiveTab("applications")}
          className={`px-6 py-2 rounded-lg text-white font-semibold transition ${
            activeTab === "applications" ? "bg-purple-600" : "bg-gray-500 hover:bg-gray-600"
          }`}
        >
          Candidate Applications
        </button>

        {/* Add Video Interview Button */}
        <button
          onClick={() => setActiveTab("video")}
          className={`px-6 py-2 rounded-lg text-white font-semibold transition ${
            activeTab === "video" ? "bg-teal-600" : "bg-gray-500 hover:bg-gray-600"
          }`}
        >
          Live Interview
        </button>
      ...
      {activeTab === "jobs" ? (
        ...
        <VideoInterview roomId={roomId} /> // Live Video Interview
      )}
    </div>
  );
}
